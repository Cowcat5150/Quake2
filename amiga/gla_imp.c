...
#ifndef __PPC__
static struct Device *TimerBase = NULL; // Cowcat
#endif
...
// Cowcat windowmode stuff & pointer

//static unsigned short *g_pCursor = 0;
struct Window *g_pWindow;

DLLFUNC void GLimp_EmptyCursor(void)
{	 
	#if 0
    	...
    	if (!g_pCursor)
	{
		...
		g_pCursor = (unsigned short *)AllocVecPPC(16, MEMF_CLEAR|MEMF_CHIP,0);

		#else

		g_pCursor = (unsigned short *)AllocVec(16, MEMF_CLEAR|MEMF_CHIP);

		#endif

		SetPointer(g_pWindow, g_pCursor, 0, 0, 0, 0);
	}
...
DLLFUNC void GLimp_EnableCursor(void)
{
	#if 0
...
DLLFUNC void GL_SetLockMode(char *mode)
{
...
qboolean VID_CreateWindow( int width, int height, qboolean fullscreen )
{
	...
	cv =        ri.Cvar_Get("gl_closeworkbench", "0", CVAR_ARCHIVE); // was 1 Cowcat

	if (cv->value)
		mglProposeCloseDesktop(GL_TRUE);

	else
		mglProposeCloseDesktop(GL_FALSE);
...
**/
	...
	*/
	...
	//if (!fullscreen)
	    //MoveWindow(mglGetWindowHandle(), x, y);  // Disabled Cowcat

	mglGetWindowHandle();	// Important call for fullscreen buffers ! Cowcat
	...
	g_pWindow = mglGetWindowHandle();  	// Cowcat
	GLimp_EmptyCursor(); 			// Cowcat
...
int GLimp_Init( void *hinstance, void *wndproc )
{
	...
	{
		TextureCache = HARD_CreateCache(cachesize, 
			(MAX_GLTEXTURES - (TEXNUM_SCRAPS - TEXNUM_LIGHTMAPS)), GLimp_TextureSwap, GLimp_TextureReload);
	}
...
#endif
{
	//extern cvar_t *gl_debug; // Cowcat
	static int timed = 0;
	static struct timeval tv;
	ULONG ms;
  	//int current,peak; 
  	//mglTexMemStat(&current,&peak); // Cowcat
  	//kprintf("Stat: %i %i\n",current,peak);
	...
	if (TextureCache && gl_texturecache->modified)
	{
		...
		{
			ri.Cvar_SetValue("gl_texturecache", 1000000);  // Cowcat fix
...
DLLFUNC void GLimp_EndFrame (void)
{
	mglUnlockDisplay();
	mglSwitchDisplay();
}
...
DLLFUNC void GLimp_AppActivate( qboolean active )
{
}
