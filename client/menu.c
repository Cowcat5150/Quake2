...
extern void MouseHandler (void);    // Cowcat windowmode check in M_ForcemenuOff/M_Menu_Main/M_Keys_Menu_f
extern void MouseHandlerOff (void); // Cowcat
extern cvar_t *vid_fullscreen;	    // Cowcat
...
// yamagi
static qboolean M_IsGame(const char *gamename)
{
...
{ 
	if (!vid_fullscreen->value)
	{
		MouseHandlerOff(); //Cowcat
		re.EnableCursor(); //Cowcat
	}
...
#ifdef AMIGAOS 
static menulist_s               s_options_sounddriver_list; 
...
{ 
    ...
    if (sensitivity) 	s_options_sensitivity_slider.curvalue   = (sensitivity->value) * 2; 
    else 	     	s_options_sensitivity_slider.curvalue = 0; 
 
    if (cl_run)      	Cvar_SetValue("cl_run", ClampCvar( 0, 1, cl_run->value )); 
    else             	Cvar_SetValue("cl_run", 0); 
 
    if (cl_run)      	s_options_alwaysrun_box.curvalue = cl_run->value; 
    else             	s_options_alwaysrun_box.curvalue = 0; 
 
    if (m_pitch)     	s_options_invertmouse_box.curvalue = m_pitch->value < 0; 
    else             	s_options_invertmouse_box.curvalue = 0; 
 
    if (lookspring)  	Cvar_SetValue("lookspring", ClampCvar( 0, 1, lookspring->value ) ); 
    else             	Cvar_SetValue("lookspring",0); 
 
    if (lookspring)  	s_options_lookspring_box.curvalue = lookspring->value; 
    else            	s_options_lookspring_box.curvalue = 0; 
 
    if (lookstrafe)     Cvar_SetValue("lookstrafe", ClampCvar( 0, 1,lookstrafe->value ) ); 
    else                Cvar_SetValue("lookstrafe",0); 
 
    if (lookstrafe) 	s_options_lookstrafe_box.curvalue = lookstrafe->value; 
    else 		s_options_lookstrafe_box.curvalue = 0; 
 
    if (freelook) 	Cvar_SetValue( "freelook", ClampCvar( 0, 1, freelook->value) ); 
    else		Cvar_SetValue("freelook",0); 
 
    if (freelook) 	s_options_freelook_box.curvalue = freelook->value; 
    else 		s_options_freelook_box.curvalue = 0; 
 
    if (crosshair) 	Cvar_SetValue( "crosshair", ClampCvar( 0, 3, crosshair->value ) ); 
    else 		Cvar_SetValue("crosshair",0); 
 
    if (crosshair) 	s_options_crosshair_box.curvalue = crosshair->value; 
    else 		s_options_crosshair_box.curvalue = 0; 
 
    if (in_joystick) 	Cvar_SetValue( "in_joystick", ClampCvar( 0, 1, in_joystick->value ) ); 
    else 		Cvar_SetValue("in_joystick",0); 
 
    if (in_joystick) 	s_options_joystick_box.curvalue = in_joystick->value; 
    else 		s_options_joystick_box.curvalue = 0; 
 
    if (win_noalttab) 	s_options_noalttab_box.curvalue = win_noalttab->value; 
    else 		s_options_noalttab_box.curvalue=0; 
} 
#endif 
...
#ifdef AMIGAOS 
static void UpdateSoundDriverFunc( void *unused ) 
{ 
    switch(s_options_sounddriver_list.curvalue) 
    { 
	case 0: 
	    Cvar_Set("s_lib", "paula"); 
	    break; 
	case 1: 
	    Cvar_Set("s_lib", "ahi"); 
	    break; 
...
{ 
	...
	#else 
	static const char *snd_driver_items[] = 
	{ 
		"paula", "ahi", "null", 0 
	}; 
	#endif 
	...
	*/ 
	...
	#else 
	s_options_sounddriver_list.generic.type         = MTYPE_SPINCONTROL; 
	s_options_sounddriver_list.generic.x            = 0; 
	s_options_sounddriver_list.generic.y            = 20; 
	s_options_sounddriver_list.generic.name         = "sound system driver"; 
	s_options_sounddriver_list.generic.callback     = UpdateSoundDriverFunc; 
	s_options_sounddriver_list.itemnames            = snd_driver_items; 
	if (Cvar_VariableString("s_lib") == 0) 
	{ 
	    Cvar_Set("s_lib", "paula"); 
	    s_options_sounddriver_list.curvalue = 0; 
	} 
	else 
	{ 
	    if (strcmp(Cvar_VariableString("s_lib"), "paula") == 0) 
		s_options_sounddriver_list.curvalue         = 0; 
	    else if (strcmp(Cvar_VariableString("s_lib"), "ahi") == 0) 
		s_options_sounddriver_list.curvalue         = 1; 
	    else 
		s_options_sounddriver_list.curvalue         = 0; 
	} 
	#endif 
...
*/ 
	...
	#else 
	Menu_AddItem( &s_options_menu, ( void * ) &s_options_sounddriver_list ); 
	#endif 
...
{ 
	...
	"", 
	"Steffen Haeuser", 
	"(SteffenH@hyperion-entertainment.com)", 
	"", 
	"Christian Michael", 
	"(ChristianM@hyperion-entertainment.com)", 
  "", 
#endif 
	"+CINEMATIC SEQUENCES", 
...
//surgeon: fixed screenheight-limitation in AMIGAOS version
...
{ 
	...
	creditsBuffer = NULL; 
	count = FS_LoadFile ("credits", (void **) &creditsBuffer); // fix Cowcat
...
//PGM 
	if((s_rules_box.curvalue < 2) || /*(Developer_searchpath(2) != 2)*/ M_IsGame("rogue") ) // yamagi 
	{ 
		Cvar_SetValue ("deathmatch", !s_rules_box.curvalue ); 
		Cvar_SetValue ("coop", s_rules_box.curvalue ); 
...
#ifdef AMIGAOS 
static qboolean PlayerConfig_ScanDirectories( int baseq2_only ) 
#else 
...
{ 
	...
	*/ 
	...
	if (baseq2_only) 
	{ 
	    Com_sprintf(findname, sizeof(findname), "baseq2/players/#?", path); 
	...
	{ 
	    ...
	    { 
		    ...
		    // Relying on sprintf to check for NULL is bad :( 
		    if (!path)      break; 
...
extern void isort(void *pArray, unsigned int nElements, unsigned int nSize, int (*compare)(const void *a, const void *b));
...
#else 
	PlayerConfig_ScanDirectories(0);

	if (s_numplayermodels == 0) 
	    PlayerConfig_ScanDirectories(1); 
#endif
...
#ifndef AMIGAOS       // Great, qsort messes up at exactly 3 elements... 
	qsort( s_pmi, s_numplayermodels, sizeof( s_pmi[0] ), pmicmpfnc ); 
#else 
	isort( s_pmi, s_numplayermodels, sizeof( s_pmi[0] ), pmicmpfnc ); 
#endif
