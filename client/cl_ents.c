...
void CL_ParseFrame (void)
{
	...
	// Knightmare- set pointer to player state for movement code  ---------to test
...
extern vec3_t clientOrg; // add
//extern char *currentweaponmodel; // for menu
//extern char cl_weaponmodels[MAX_CLIENTWEAPONMODELS][MAX_QPATH];
extern cvar_t *cl_item_bob; // bob !
...
void CL_AddPacketEntities (frame_t *frame)
{
	...
	cvar_t *game = Cvar_Get("game", "", CVAR_LATCH | CVAR_SERVERINFO); // yamagi - rogue path

	// bonus items rotate at a fixed rate
	autorotate = anglemod(cl.time * 0.1); // was /10 - Yamagi
	...
	VectorCopy (vec3_origin, clientOrg); // Knightmare 
	//currentweaponmodel = NULL;

	for (pnum = 0 ; pnum<frame->num_entities ; pnum++)
	{
		qboolean isclientviewer = false; // Knightmare
		...
		// if i want to multiply alpha, then id better do this... Knightmare
		ent.alpha = 1.0;
		...
		// ent that cast light don´t give off shadows - Knightmare
...
//============
		...
		if (s1->number == cl.playernum + 1 )
		{
			...
			isclientviewer = true;		// Knightmare added
			...
			#if 0 // test for weapon menu model ?
			...
			if(!cl_3dcam->value) // Knightmare added
				continue;
		}
...
//pm
		...
		// if set to invisible, skip
		if(!s1->modelindex)	// Knightmare
			continue;
		
		// add to refresh list
		if(drawEnt)		// Knightmare
			V_AddEntity (&ent);

		// color shells generate a seperate entity for the main model
		if (effects & EF_COLOR_SHELL && (!isclientviewer || cl_3dcam->value)) // knightmare added
		{
			// PMM - at this point, all of the shells have been handled
			// if we're in the rogue pack, set up the custom mixing, otherwise just
			// keep going

			if( strcmp(game->string, "rogue") == 0 ) // yamagi - re-arranged (check rogue only once !) Knightmare
			{
		...
		ent.skinnum = 0;
		ent.flags = 0;
		ent.alpha = 1.0;  	// Knightmare - was 0 Cowcat 
...
extern void vectoangles2(vec3_t value1, vec3_t angles); //added Cowcat
...
{
	// 3dcam from quakesrc.org tutorial

	vec3_t end, oldorg, camposition;
...
static float AdaptFov (float fov, float w, float h)
{
	#if 1
	...
	a = atan((w/h)/(4.0/3.0) * tan (fov/360*M_PI)) * 360/M_PI;
...
void CL_CalcViewValues (void)
{
	...
	if ((cl_predict->value) && !(cl.frame.playerstate.pmove.pm_flags & PMF_NO_PREDICTION))
	{
		...
		for (i=0 ; i<3 ; i++)
		{
			...
			cl.predicted_origin[i] -= backlerp * cl.prediction_error[i]; // Knightmare added
		}
		...
		if (delta < 100)
		{
			...
			cl.predicted_origin[2] -= cl.predicted_step * (100 - delta) * 0.01; // Knightmare added
		}
