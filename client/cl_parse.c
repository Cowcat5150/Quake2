...
void CL_ParseServerData (void)
{
	...
	if (cl.playernum == -1)
	{
		...
		//#ifndef AMIGAOS // never used own q2 cinema functions - bypass vidplay.dll - Cowcat
		#if 1
		...
		else
		{
			...
			extern unsigned int id_logo_played;

			if (strstr(str,"idlog.cin") && id_logo_played)
			{
				S_Init();
				CL_RegisterSounds();
				SCR_FinishCinematic();

				if ((system_result == 1) && (strstr(str,"idlog")))
				{
					Key_Event(K_ESCAPE, true, 0, -1);
					SV_KillServer_f();
					do_not_show_error = 1;
				}
			...
			sprintf(buffer, "vidplay >nil: progdir:%s/video/", FS_Gamedir());
...
void CL_LoadClientinfo (clientinfo_t *ci, char *s)
{
	...
	else
	{
		...
		*t = 0;
		...
		{
			// see if the skin exists for the male model
			Com_sprintf (skin_filename, sizeof(skin_filename), "players/%s/grunt.pcx", model_name /*, skin_name*/ ); // yamagi fix
			ci->skin = re.RegisterSkin (skin_filename);
		}
