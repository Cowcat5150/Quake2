...
void CL_ParseTEnt (void)
{
	...
	switch (type)
	{
		...
		case TE_GRENADE_EXPLOSION_WATER:
			...
			VectorCopy (pos, ex->ent.origin);
			ex->type = ex_poly;
			ex->ent.flags = RF_FULLBRIGHT | RF_NOSHADOW; // shadow flag
			ex->start = cl.frame.servertime - 100;
			ex->light = 350;
			ex->lightcolor[0] = 1.0;
			ex->lightcolor[1] = 0.5;
		...
		case TE_BFG_EXPLOSION:
			...
			VectorCopy (pos, ex->ent.origin);
			ex->type = ex_poly;
			ex->ent.flags = RF_FULLBRIGHT | RF_NOSHADOW; // shadow flag - new Cowcat
			ex->start = cl.frame.servertime - 100;
			ex->light = 350;
			ex->lightcolor[0] = 0.0;
			ex->lightcolor[1] = 1.0;
...
//PGM
		...
		case TE_BUBBLETRAIL2:
			...
			MSG_ReadPos (&net_message, pos);
			MSG_ReadPos (&net_message, pos2);
			CL_BubbleTrail2 (pos, pos2, /*cnt*/ 8); // fix
			S_StartSound (pos,  0, 0, cl_sfx_lashit, 1, ATTN_NORM, 0);
			break;
...
void CL_AddBeams (void)  // modded for 3dviewcam
{
	int		i, j;
	beam_t		*b;
	vec3_t		dist, org;
...
void CL_AddPlayerBeams (void) // mod 3dcam
{
	int		i, j;
	beam_t		*b;
	vec3_t		dist, org;
