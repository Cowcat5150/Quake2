...
cdlight_t *CL_AllocDlight (int key)
{
	...
	if (key)
	{
		...
		for (i=0 ; i<MAX_DLIGHTS ; i++, dl++)
		{
			if (dl->key == key)
			{
				//memset (dl, 0, sizeof(*dl)); // not needed? - new Cowcat
				//dl->key = key; //
				return dl;
			}
		}
...
void CL_ParseMuzzleFlash2 (void) 
{
	...
	origin[2] = cl_entities[ent].current.origin[2] + forward[2] *
		monster_flash_offset[flash_number][0] + right[2] * monster_flash_offset[flash_number][1] + monster_flash_offset[flash_number][2];
...
void CL_FlyParticles (vec3_t origin, int count)
{
	...
	if (!avelocities[0][0])
	{
		...
		for (i=0 ; i<NUMVERTEXNORMALS ; i++) // new Cowcat
		{
