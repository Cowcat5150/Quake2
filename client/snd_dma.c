...
sfx_t *S_FindName (char *name, qboolean create)
{
	...
	for (i=0 ; i < num_sfx ; i++)
	{
		if (!known_sfx[i].name[0]) // registration_sequence < s_registration_sequence)
			break;
	}
	...
	sfx = &known_sfx[i];
	sfx->truename = NULL; // memset (sfx, 0, sizeof(*sfx)); // new Cowcat
...
sfx_t *S_AliasName (char *aliasname, char *truename)
{
	...
	sfx = &known_sfx[i];
	sfx->cache = NULL; // memset (sfx, 0, sizeof(*sfx)); // new Cowcat
...
struct sfx_s *S_RegisterSexedSound (entity_state_t *ent, char *base)
{
	...
	if (!sfx)
	{
		...
		if (f)
		{
			...
			//FS_FCloseFile (f); // disabled - new Cowcat
...
void S_EndRegistration (void)
{
	...
	for (i=0, sfx=known_sfx ; i < num_sfx ; i++,sfx++)
	{
		...
		if (sfx->registration_sequence != s_registration_sequence)
		{
			...
			//memset (sfx, 0, sizeof(*sfx));
			if(sfx->truename) // new Cowcat
				Z_Free(sfx->truename);
			
			sfx->cache = NULL;  // new Cowcat
			sfx->name[0] = 0;  // new Cowcat
		}
