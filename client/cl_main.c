...
cvar_t	*cl_drawfps; 	// Cowcat
cvar_t	*cl_3dcam; 	// 3dcam
cvar_t	*cl_3dcam_angle;
cvar_t  *cl_3dcam_dist;
cvar_t  *cl_3dcam_alpha;

cvar_t	*horplus; // Cowcat
cvar_t	*cl_item_bob; //
...
//extern int putenv (const char *buffer); // Cowcat
...
//      cl_minfps = Cvar_Get ("cl_minfps", "5", 0);
	...
	cl_drawfps = Cvar_Get("cl_drawfps","0", CVAR_ARCHIVE); 		// added Cowcat
	cl_3dcam =  Cvar_Get("cl_3dcam", "0", CVAR_ARCHIVE);		// 3dcam
	...
	horplus = Cvar_Get ("horplus","0", CVAR_ARCHIVE); // Cowcat
	cl_item_bob = Cvar_Get ("cl_item_bob", "0", CVAR_ARCHIVE); // item bobbing
...
unsigned int id_logo_played = 0;

void PlayIdLogo()
{
	#if 1 // bypass vidplay.dll - Cowcat (2017)
	...
	// allow to continue game without videos - Cowcat
	Com_sprintf (name, sizeof(name), "video/%s", str);
	...
	char str[] = "idlog.cin";
		...
		return;
      	...
      	if (ahi_speed)
		ahi_speed_val=ahi_speed->value;

      	if (ahi_channels)
		ahi_channels_val=ahi_channels->value;

      	if (ahi_bits)
		ahi_bits_val=ahi_bits->value;

      	if (khz)
		khz_val=khz->value;

      	if (vid_gamma)
		gamma_val=vid_gamma->value;

      	else
		gamma_val=0.7;

      	if (s_volume)
		volume_val=s_volume->value;

      	else
		volume_val=0.7;

      	if (s_lib)
		strncpy(soundlib,s_lib->string,256);

      	else
		strcpy(soundlib,"paula");
...
{	
	...
	if (isdown)
	{
		#ifndef AMIGAOS	// Prevents window popping up after start from Workbench...
