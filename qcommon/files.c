...
extern void Sys_ConvertFilename(char *filename); // Cowcat
...
#ifdef AMIGAOS
int FS_FOpenFile (char *file_name, FILE **file)
#else
		...
		if (search->pack)
		{
			...
			for (i=0 ; i<pak->numfiles ; i++)
				if (!Q_strcasecmp (pak->files[i].name, filename))
				{
					...
					Com_sprintf (netpath, sizeof(netpath), "%s", pak->filename);
					Sys_ConvertFilename(netpath);
...
{	
	...
	for (i=0 ; i<pak->numfiles ; i++)
		...
		if (!Q_strcasecmp (pak->files[i].name, netpath))
		{
		       // found it!
			file_from_pak = 1;
			Com_DPrintf ("PackFile: %s : %s\n",pak->filename, netpath);
...
pack_t *FS_LoadPackFile (char *packfile)
{
	...
	char filename[MAX_OSPATH];
	strcpy(filename, packfile);
	Sys_ConvertFilename(filename);

	packhandle = fopen(filename, "r");
...
void FS_ExecAutoexec (void)
{
	...
	if (*dir)
	    Com_sprintf(name, sizeof(name), "%s/autoexec.cfg", dir);

	else
	    Com_sprintf(name, sizeof(name), "%s/autoexec.cfg", BASEDIRNAME);
