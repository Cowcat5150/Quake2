...
#include "gl_local.h"
#include "../amiga/renderercache.h"

#define ORDNUM(image) (image-gltextures)

extern		qboolean arb_multitexture;
...
void GL_Bind (int texnum)
{
   	...
   	if(TextureCache)
   	{
		...
		ima = image = &gltextures[0];

		for (ima=gltextures,i=0; i<numgltextures; i++,ima++)
		{
			if (ima->texnum==texnum)
				image=&gltextures[i];
		}

		if (!strstr(image->name,"***"))
			HARD_AccessMember(TextureCache, ORDNUM(image));

		#else //surgeon: this should be MUCH faster

		if(texnum < TEXNUM_LIGHTMAPS || texnum > TEXNUM_SCRAPS) 
		{
			image = &gltextures[0];
			ima = gltextures;
			i = 0;
...
nonscrap:
	...
	if(TextureCache)
	{
		image->is_swapped = false;

		if ((!strstr(image->name,"***"))&&(!bypass_lru))
		{
			HARD_AddMember(TextureCache, ORDNUM(image), image->width*image->height, (void *)image);
		}
	}
...
void GL_LoadPic2(image_t *image,const char *name, byte *pic, int width, int height, imagetype_t type, int bits)
{
	image->scrap = false;
	...
	gl_state.currenttextures[gl_state.currenttmu] = image->texnum;
	qglBindTexture (GL_TEXTURE_2D, image->texnum);

	if (bits == 8)
		image->has_alpha = GL_Upload8 (pic, width, height, (type != it_pic && type != it_sky), type == it_sky );

	else
		image->has_alpha = GL_Upload32 ((unsigned *)pic, width, height, (type != it_pic && type != it_sky) );
}

image_t *GL_LoadWal2(char *name, image_t *image)  // removed const from char Cowcat
{
	miptex_t	*mt;
	...
	ofs = LittleLong (mt->offsets[0]);
  	...
  	GL_LoadPic2(image, name, (byte *)mt + ofs, width, height, it_wall, 8);
...
void GL_RestoreImage(image_t *image)
{
  	int 	len;
  	byte 	*pic, *palette;
  	int 	width,height;
	...
	//kprintf("Data: %s\n",image->name);
  	...
  	if (!strstr(image->name,"***"))
		HARD_RemoveMember(TextureCache,ORDNUM(image));

  	if (image->is_swapped == false)
	...
	image->is_swapped = false;
  	...
  	if (!strcmp(image->name+len-4, ".pcx"))
  	{
    		...
    		if (!pic) return;
    			GL_LoadPic2(image,image->name,pic,width,height,image->type,8);
  	}

  	else if (!strcmp(image->name+len-4, ".wal"))
  	{
    		image = GL_LoadWal2(image->name,image);
  	}

  	else if (!strcmp(image->name+len-4, ".tga"))
  	{
		...
		GL_LoadPic2(image,image->name, pic, width, height, image->type, 32);
  	}
...
void GL_InitImages (void)
{
	...
	if ( qglColorTableEXT )
	{
		ri.FS_LoadFile( "pics/16to8.dat", (void **) &gl_state.d_16to8table ); // fix Cowcat

		if ( !gl_state.d_16to8table )
			ri.Sys_Error( ERR_FATAL, "Couldn't load pics/16to8.pcx");
	}

	#ifndef AMIGAOS //surgeon: why no gamma-adjust for voodoo ? 
