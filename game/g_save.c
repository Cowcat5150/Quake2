		...
		case F_FUNCTION:
			...
			else
			{
				...
				{	
					...
					gi.dprintf ("WriteField1: warning - function not in list\n"); // workaround - Cowcat
					len = 0; //
				}
		...
		case F_FUNCTION:
			...
			if (*(byte **)p)
			{
				...
				if (!func)
				{
					...
					gi.dprintf ("WriteField2: warning - function not in list\n"); // workaround - Cowcat
				}
...
DLLFUNC void WriteGame(const char *filename, qboolean autosave)
{
	FILE *f;
	int i;
	char str_ver[32];
...
DLLFUNC void ReadGame(const char *filename)
{
	FILE *f;
	int i;
	char str_ver[32];
...
DLLFUNC void WriteLevel(const char *filename)
{
	int i;
	edict_t *ent;
	FILE *f;
...
DLLFUNC void ReadLevel(const char *filename)
{
	int entnum;
	FILE *f;
	int i;
