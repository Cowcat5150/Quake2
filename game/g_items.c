...
void droptofloor (edict_t *ent)
{
	...
	if (tr.startsolid)
	{
		gi.dprintf ("droptofloor: %s startsolid at %s\n", ent->classname, vtos(ent->s.origin)); // enabled now - Cowcat
		G_FreeEdict (ent);
		return;
	}
...
void InitItems (void)
{
	//memset(itemlist, 0, sizeof(itemlist)); // new Cowcat
	//memcpy(itemlist, 0, gameitemlist, sizeof(gameitemlist)); //
	//game.num_items = sizeof(gameitemlist) / sizeof(gameitemlist[0]) - 1; //

	game.num_items = sizeof(itemlist) / sizeof(itemlist[0]) - 1; // old
}
