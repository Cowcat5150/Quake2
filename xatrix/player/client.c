...
SelectSpawnPoint(edict_t *ent, vec3_t origin, vec3_t angles)
{
	...
	if (coop->value)
	{
		...
		if (Q_strcasecmp(spot->classname, "info_player_start") == 0 && index != 0)
		{
			while(counter < 3)
			{
...
DLLFUNC trace_t PM_trace(vec3_t start, vec3_t mins, vec3_t maxs, vec3_t end) // Hidden DLLFUNC (again)!! Cowcat
{
...
DLLFUNC void ClientThink(edict_t *ent, usercmd_t *ucmd)
{
	gclient_t *client;
	edict_t *other;
	int i, j;
