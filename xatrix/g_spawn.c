...
qboolean Spawn_CheckCoop_MapHacks (edict_t *ent)
{
	...
	if(!Q_strcasecmp(level.mapname, "xsewer1"))
	{
		if(ent->classname && !Q_strcasecmp(ent->classname, "trigger_relay") && ent->target && !Q_strcasecmp(ent->target, "t3") && ent->targetname && !Q_strcasecmp(ent->targetname, "t2"))
		{
			return true;
		}
		if(ent->classname && !Q_strcasecmp(ent->classname, "func_button") && ent->target && !Q_strcasecmp(ent->target, "t16") && ent->model && !Q_strcasecmp(ent->model, "*71"))
		{
			ent->message = "Overflow valve maintenance\nhatch A opened.";
			return false;
		}

		if(ent->classname && !Q_strcasecmp(ent->classname, "trigger_once") && ent->model && !Q_strcasecmp(ent->model, "*3"))
		{
			ent->message = "Overflow valve maintenance\nhatch B opened.";
			return false;
		}
...
DLLFUNC void SpawnEntities(const char *mapname, char *entities, const char *spawnpoint)
{
	edict_t *ent;
	int inhibit;
	const char *com_token;
	...
	while (1)
	{
		...
		if (!Q_strcasecmp(level.mapname, "command") &&
			!Q_strcasecmp(ent->classname, "trigger_once") &&
		   	!Q_strcasecmp(ent->model, "*27"))
		{
