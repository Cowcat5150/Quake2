...
model_t *Mod_ForName (char *name, qboolean crash)
{
	...
	modfilelen = ri.FS_LoadFile (mod->name, (void **) &buf); // fix Cowcat
...
void Mod_LoadFaces (lump_t *l)
{
	...
	for ( surfnum=0 ; surfnum<count ; surfnum++, in++, out++)
	{
		...
		for (i=0 ; i<MAXLIGHTMAPS ; i++)
			out->styles[i] = in->styles[i]; // see down

		//memcpy (out->styles, in->styles, sizeof(byte) * MAXLIGHTMAPS); // future test Cowcat
		...
		if (out->texinfo->flags & SURF_SKY) // was missing - Cowcat 2019
...
DLLFUNC void R_BeginRegistration (char *model)
{
