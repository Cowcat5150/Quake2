...
#include "shared.h"
#include <stddef.h>   // added for vbcc - Cowcat
...
#define	FOFS(x) (size_t)offsetof(edict_t,x)
#define	STOFS(x) (size_t)offsetof(spawn_temp_t,x)
#define	LLOFS(x) (size_t)offsetof(level_locals_t,x)
#define	CLOFS(x) (size_t)offsetof(gclient_t,x)

// Cowcat added defines

#define FOFSES(x,y) (size_t)FOFS(x)+offsetof(entity_state_t,y)
#define FOFSMO(x,y) (size_t)FOFS(x)+offsetof(monsterinfo_t,y)
#define FOFSMV(x,y) (size_t)FOFS(x)+offsetof(moveinfo_t,y)
#define CLOFSP(x,y) (size_t)CLOFS(x)+offsetof(client_persistant_t,y)
...
#define random() ((rand() & 0x7fff) / ((float)0x7fff)) // Cowcat
#define crandom() (2.0 * (random() - 0.5)) // Cowcat
...
/* g_svcmds.c */
DLLFUNC void ServerCommand(void);
