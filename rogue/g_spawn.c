		...
		if (!(f->flags & FFL_NOSPAWN) && !Q_strcasecmp(f->name, (char *)key))
		{
			...
			switch (f->type)
			{
				...
				case F_INT:
					...
					*(int *)(b + f->ofs) = atoi(value); // Cowcat
					break;

				case F_FLOAT:
					//*(float *)(b + f->ofs) =  strtof(value, (char **)NULL);;
					*(float *)(b + f->ofs) = atof(value); // Cowcat
					break;
...
DLLFUNC void SpawnEntities(const char *mapname, char *entities, const char *spawnpoint)
{
	...
	while (1)
	{
		...
		if (!Q_strcasecmp(level.mapname, "command") &&
			!Q_strcasecmp(ent->classname, "trigger_once") && !Q_strcasecmp(ent->model, "*27"))
		{
			ent->spawnflags &= ~SPAWNFLAG_NOT_HARD;
		}

		/* ahh, the joys of map hacks .. */
		if (!Q_strcasecmp(level.mapname, "rhangar2") &&
			!Q_strcasecmp(ent->classname, "func_door_rotating") &&
			ent->targetname && !Q_strcasecmp(ent->targetname, "t265"))
		{
			ent->spawnflags &= ~SPAWNFLAG_NOT_COOP;
		}

		if (!Q_strcasecmp(level.mapname, "rhangar2") &&
			!Q_strcasecmp(ent->classname, "trigger_always") &&
		   	ent->target && !Q_strcasecmp(ent->target, "t265"))
		{
			ent->spawnflags |= SPAWNFLAG_NOT_COOP;
		}

		if (!Q_strcasecmp(level.mapname, "rhangar2") &&
			!Q_strcasecmp(ent->classname, "func_wall") &&
		   	!Q_strcasecmp(ent->model, "*15"))
		{
			ent->spawnflags |= SPAWNFLAG_NOT_COOP;
		}
