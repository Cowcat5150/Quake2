...
cvar_t	*sw_waterwarp;
cvar_t	*sw_custom_particles; // Cowcat new
cvar_t	*r_drawworld;
...
void R_Register (void)
{
	...
	sw_custom_particles = ri.Cvar_Get ("sw_custom_particles", "0", CVAR_ARCHIVE); // new Cowcat
...
DLLFUNC qboolean R_Init( void *hInstance, void *wndProc )
{
	R_InitImages ();
...
static void R_DrawBEntitiesOnList (void)
{
	...
	insubmodel = true;
	//r_dlightframecount = r_framecount; // Cowcat removed - see r_light.c
...
static void R_CalcPalette (void)
{
...
DLLFUNC void R_CinematicSetPalette( const unsigned char *palette )
{
	byte 	palette32[1024];
	int	i, j, w;
	int	*d;
...
DLLFUNC refexport_t GetRefAPI (refimport_t rimp)
{
	...
	re.BeginRegistration = R_BeginRegistration;   	// r_model.c
    	re.RegisterModel = R_RegisterModel;	      	//    "
    	re.RegisterSkin = R_RegisterSkin;             	// r_main.c/r_image.c
	re.RegisterPic = Draw_FindPic;       		// r_draw.c
	re.SetSky = R_SetSky;                		// r_main.c
	re.EndRegistration = R_EndRegistration;       	// r_model.c

	re.RenderFrame = R_RenderFrame;   	// r_main.c

	re.DrawGetPicSize = Draw_GetPicSize; 	// r_draw.c
	re.DrawPic = Draw_Pic;  	     	//  "
	...
	re.Init = R_Init;	   // r_main.c
	re.Shutdown = R_Shutdown;  //   "

	re.CinematicSetPalette = R_CinematicSetPalette; // r_main.c
	re.BeginFrame = R_BeginFrame;                   //    "
	re.EndFrame = SWimp_EndFrame;	      // sw_imp.c

	re.AppActivate = SWimp_AppActivate;   //    "

	re.EmptyCursor = SWimp_EmptyCursor;  // Cowcat
	re.EnableCursor = SWimp_EnableCursor; // Cowcat
