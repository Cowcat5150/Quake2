...
int		d_pix_min, d_pix_max, d_pix_mul; // d_pix_shift; - Cowcat new _mul
...
void D_ViewChanged (void)
{
	...
	d_pix_min = r_refdef.vrect.height / 240; // new Cowcat

	if (d_pix_min < 1)
		d_pix_min = 1;

	d_pix_max = (int)((float)r_refdef.vrect.height / (240.0 / 4.0) + 0.5); // new Cowcat
	
	if (d_pix_max < 1)
		d_pix_max = 1;

	d_pix_mul = (int)((float)r_refdef.vrect.height / 240.0 + 0.5); // was 8 - (int)(( new Cowcat
...
void R_SetupFrame (void)
{
	...
	if (r_dowarp)
	{
		...
		//vrect.width = r_newrefdef.width; // new Cowcat
		//vrect.height = r_newrefdef.height; //

		d_viewbuffer = r_warpbuffer;
		r_screenwidth = WARP_WIDTH;
		//r_screenwidth = vid.width; // new Cowcat
	}
