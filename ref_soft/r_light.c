...
//int	r_dlightframecount; // was used (??) in r_main.c/drawbentitiesonlist - Cowcat
...
static void R_MarkLights (dlight_t *light, int bit, mnode_t *node, int r_dlightframecount)
{
	mplane_t	*splitplane;
	float		dist;
	msurface_t	*surf;
...
void R_PushDlights (model_t *model)
{
	...
	//r_dlightframecount = r_framecount; // Cowcat

	for (i=0, l = r_newrefdef.dlights ; i<r_newrefdef.num_dlights ; i++, l++)
	{
		R_MarkLights ( l, 1<<i, model->nodes + model->firstnode, r_framecount); // framecount - Cowcat new
	}
}
...
static void R_AddDynamicLights (void)
{
	...
	for (lnum=0 ; lnum<r_newrefdef.num_dlights ; lnum++)
	{
		light_t *plightdest = blocklights; // new Cowcat
